...
public class MissileTargetHighScoreRouletteWheelSelectionLogic implements SelectionLogic {
	private var _percent:Number;
	private var _deduplication:Boolean;

	public function MissileTargetHighScoreRouletteWheelSelectionLogic(percent:Number, deduplication:Boolean) {
	...
	public function getPieChart(entities:Vector.<MissileTargetEntity>,mc:MovieClip=null) {
		...
		for (var i:int = 0; i < entities.length; i++) {
			...
			var currentEntity:MissileTargetEntity =entities[i];
			var expression:String =  "["+currentEntity.getTotalGeneFitness()+"]("+fPercent+"%)";
	...
	private function getRoulettePlay(entities:Vector.<Entity>, baseNumber:Number, entityTotal:Number):Entity {
		...
		for (var i:int = 0; i < entities.length; i++) {
			...
			if (rand <= randLoop) {
				...
				var castedEntity:MissileTargetEntity = MissileTargetEntity(entities[i]);
				...
				var newEntity:MissileTargetEntity = new MissileTargetEntity(copyInformation, castedEntity.getFitnessMeasurement());
				return newEntity;
			}
		}
		trace("IT's NULL");
