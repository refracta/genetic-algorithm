...
public class BinarySinglePointCrossoverLogic implements CrossoverLogic {
	...
	public Entity createCrossoverEntity(Entity e1, Entity e2) {
		...
		int halfIndex = (binaryEntity1.getGeneInformation().size()-1)/2 +1;

		if(binaryEntity1.getGeneInformation().size()<=1){
			try {
			...
			for (int i = 0; i < halfIndex; i++) {
				binaryGenes.add((BinaryGene) binaryEntity1.getGeneInformation().get(i));
			}
			for (int i = halfIndex; i < binaryEntity2.getGeneInformation().size(); i++) {
				binaryGenes.add((BinaryGene) binaryEntity2.getGeneInformation().get(i));
			}
		}
