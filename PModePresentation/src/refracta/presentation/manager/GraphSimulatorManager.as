...
public class GraphSimulatorManager {
	...
	public function GraphSimulatorManager(locationX:Number, locationY:Number, swfWidth:Number, swfHeight:Number) {
	...
	public function loadStart() {
		if (!PresentationConst.GRAPH_SIMULATOR_CACHE_LOCATION.exists) {
			trace("Load From Server (GRAPH SIMULATOR)");
			this.dataLoader.load(PresentationConst.GRAPH_SIMULATOR_URL);
		} else {
			trace("Load From File (GRAPH SIMULATOR)");
			this.dataLoader.load(new URLRequest(PresentationConst.GRAPH_SIMULATOR_CACHE_LOCATION.url));
		}
	}

	private function progressHandler(event:ProgressEvent):void {
		var percentLoaded:int = event.target.bytesLoaded / event.target.bytesTotal * 100;
		var callName:String = "GraphSimulator";
	...
	public function addSwf() {
		var swfLoader:SWFLoader = new SWFLoader("resources/" + PresentationConst.GRAPH_SIMULATOR_FILE_NAME, {
			container: _viewClip,
			x: locationX,
			y: locationY,
			width: swfWidth,
	...
	private function saveSwfBytes(event:*) {
		...
		fileStream.open(PresentationConst.GRAPH_SIMULATOR_CACHE_LOCATION, FileMode.WRITE);
